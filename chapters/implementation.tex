...
required by the protocol. This did not change any functionality of the design, and would never show up in simulation unless a master or agent was sensitive to this. The correct zero cycle okay response was added to simplify the completeness proof. \TLSAY{Lot's of detail ... not sure how to put this in context} 
...
status is not reported back to the master, any error would only be known locally in the agent. \TLSAY{Just explain the feedback path ... no need to discuss pros and cons here} It would be possible to assert an error signal that is checked 
...
\end{enumerate}
\TLSAY{This should be very close to the graphics... that's the reason for this section isn't it?} 
...
\TLSAY{I liked the enum with an explanation for each state. Maybe you can do it here too? In general I'm somewhat confused. You're talking about the implementation here? You're always mixing in Property stuff ... I don't like it. Why not explain the implementation. Then explain the properties with examples and pinpoint specific things?}

 The transfer payload is written out on the slave out port and the slave processes the request. The slave responds with status, and data in the case of a read. If the status is error the slave agent inserts an error cycle to provide the two cycle error response. \par
...
\TLSAY{I'd flip the statement here. Because there is an existing implementation it makes sense to start bottom-up and fit a SystemC-PPA to the existing implementation.} 

\subsection{see below}
\TLREP{Manual property set}{Bottom-up property creation}

\TLSAY{This reads more like a report of the steps you took. But the aim here is that you should explain how a property set can be extracted bottom up. What's the general idea? Where to start. Explain the easy stuff. Then make a paragraph on the complicated things. Your text reads like this: I did this, then there was this problem, so I did this, well then there was another issue, so I had to do that ... Instead you'll should go like this "In order to extract properties bottom-up you first do a (with the goal x), then b (with goal x), then c ... Following this schematic the following problems can occur ... Due to the some special things of the protocol step 2 was complex because (explain the problem). 

That counts for all sections in this chapter.

Furthermore, I'm missing graphics and property examples. This is way too much text. I'll get bored and confused really quick. 

I'll gives this section another read once you reworked. 
  } 
