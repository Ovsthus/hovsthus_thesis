...
\TLSAY{I think you're talking about a specific property? First remove the blue color that's confusing. Second add a simplified pseudo property explaining what you're doing. First you explain the property, then you make the point you want to make. I'm a little lost here. Please mark me this section for re-read} 
...
\TLSAY{Don't post source code as text. Post it as figure} 


The communication with the emulated port must occur exactly once in all three states. It is what signals the important states 1,2 and 5 from Sec.~\ref{bottom-up}. It is required to be a non-blocking write to unconditionally call a wait, which hands over control to the emulated port before it "gets" the requests. \ITLKW{true} and \textit{sync} offer no function but are required for the function call. \TLSAY{blue color ... are you sure you can pass a boolean value instead of a variable to a read funtion? If this works, then it is a bug. Why would you not post a reduced version of the source code for the matrix/agent? and explain things by referencing line of codes. The thesis is lacking code/property examples.}  \par
...
\TLSAY{The slave error response thing needs example/figures code and a why and how. Kinda got lost here again. Maybe you can rework the second half of the chapter?} 
...
\TLSAY{This makes no sense here .. just posting a single line. we need the entire code} 
...
The ESL model has no notion of time, which entails that the modeling of starvation is not inherently equal to the RTL. Because it is an untimed model, the y-axis of the plot is not delay in time, rather it is the amount of wait functions called. \par
The bus matrix differentiate between read, write and error without interrupting, or otherwise delaying the completion of the transaction. The bus matrix does differentiate between in and out of range transfers, in a similar manner to the RTL model, provided that the wait function is called with a non-zero wait time. 
